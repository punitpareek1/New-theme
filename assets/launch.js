...
/* harmony default export */ const pxs_video_dist_index_es = (PxsVideo);
...
;// CONCATENATED MODULE: ./source/scripts/templates/TemplateAddresses.js

class TemplateAddresses {
  constructor(section) {
  ...
  _bindEvents() {
    ...
    jquery_default()('.cancel-edit', this.$el).on('click', () => this.on());
    jquery_default()('.toggle-new-address', this.$el).on('click', () => this.toggleNewAddress());
    jquery_default()('.select-wrapper select', this.$el).on('click', () => this.updateSelectedText());
  }
  prepareAddresses() {
    ...
    selectableOptions.each((index, select) => this.updateSelectedText(null, select));
  }
  updateSelectedText(event, selectedText) {
    const select = event ? jquery_default()(event.target) : jquery_default()(selectedText);
    const selectedValue = select.find('option:selected').text();
    if (selectedValue !== '') {
      select.prev('.selected-text').text(selectedValue);
    }
    if (select.attr('name') === 'address[country]') {
      let addressID = jquery_default()(select).attr('id').split('address-country-')[1];
  ...
  deleteAddress(event) {
    ...
    const addressID = jquery_default()(event.target).parents('[data-address-id]').data('address-id');
  ...
  editAddress(event) {
    ...
    const addressID = jquery_default()(event.currentTarget).parents('[data-address-id]').data('address-id');
...
;// CONCATENATED MODULE: ./source/scripts/components/BlogPost.js

class BlogPost {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.setFeaturedImage();
    this.artDirection();
    this.wrapAllNodes();
    jquery_default()('.highlight').each((index, highlight) => {
      this.fixOverlappingElements(jquery_default()(highlight));
    });
  ...
  wrapAllNodes() {
    const {
      childNodes
    } = jquery_default()('.rte', this.$el)[0];
    jquery_default().each(childNodes, (index, node) => {
      if (node.nodeType === 3 && node.textContent.replace(/^\s+|\s+$/g, '')) {
        jquery_default()(node).replaceWith(`<p>${node.textContent}</p>`);
      }
    });
  }
  fixOverlappingElements(highlight) {
    if (jquery_default()('.post-meta').overlaps(highlight).length) {
      highlight.addClass('overlapping');
    }
    highlight.addClass('processed');
  }
  setFeaturedImage(resize) {
    const featuredImage = jquery_default()('.featured-image', this.$el);
    if (featuredImage.length) {
      const parent = featuredImage.parent();
      const windowWidth = jquery_default()(window).width();
      const contentWidth = jquery_default()('.post-content', this.$el).width();
      if (resize) {
        featuredImage.css({
          width: windowWidth,
  ...
  artDirection() {
    const images = jquery_default()('.post-content', this.$el);
    jquery_default()(images[0]).one('rimg:load', () => {
      images.each((index, value) => {
        const image = jquery_default()(value);
        ...
        if (image.css('float') !== 'none') {
          ...
          this.fixOverlappingElements(imageParent);
        }
        ...
        if (imageAlt && imageAlt.length && imageParent.not('img')) {
          ...
          imageParent.append(`<div style='max-width: ${imageWidth}px; margin-left: ${marginLeft}; margin-right: ${marginRight};' class='photo-caption meta'>${imageAlt}</div>`);
        }
      });
    });
  }
...
class DynamicBlogPosts {
  constructor() {
    jquery_default()('.blog-post').map(post => new BlogPost({
      el: post
    }));
  }
}
;// CONCATENATED MODULE: ./source/scripts/sections/DynamicImagesWithText.js

class DynamicImagesWithText {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.$productDetailsSlideshow = jquery_default()('.product-details-slideshow-nav-list-item', this.$el);
    this.$productDetailsSlideshow.on('click.images-with-text', event => {
      event.preventDefault();
      this.specificSlide(event);
    });
    this.slidePagination = jquery_default()('.product-details-slideshow-nav-list', this.$el);
    this.slidePagination.children().first().addClass('active');
    this.$el.find('.product-details-slideshow-list-item').first().addClass('active');
  }
  specificSlide(event) {
    if (!jquery_default()(event.currentTarget).hasClass('active')) {
      const index = jquery_default()(event.currentTarget).attr('data-position');
      this.updateSlidePagination(index);
    }
  }
  ...
  showNewSlide(index) {
    this.$el.find(`.product-details-slideshow-list-item[data-position='${index}']`).addClass('active').focus().siblings().removeClass('active');
  }
  onSectionBlockSelect(block) {
    const index = jquery_default()(block.el).index() + 1;
    this.updateSlidePagination(index);
  }
  onSectionUnload() {
    this.$productDetailsSlideshow.off('.images-with-text');
  }
}
;// CONCATENATED MODULE: ./source/scripts/sections/DynamicTestimonial.js

class DynamicTestimonial {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.slideList = jquery_default()('.dynamic-testimonials-slides-list', this.$el);
    this.pageList = jquery_default()('.dynamic-testimonials-pagination-list', this.$el);
    this.$window = jquery_default()(window);
    this.$navigationItem = jquery_default()('.dynamic-testimonials-navigation-item a', this.$el);
    const slide = this.slideList.children();
    const count = this.slideList.children().length;
    this.events = [jquery_default()('.dynamic-testimonials-pagination-list-item').on('click.testimonials', event => {
      event.preventDefault();
      this.paginate(event);
    }), jquery_default()('.dynamic-testimonials-pagination-item a').on('click.testimonials', event => {
      event.preventDefault();
      this.navigate(event);
    }), jquery_default()('.dynamic-testimonials-navigation-item .icon-arrow-right').on('click.testimonials', event => {
      event.preventDefault();
      this.navigate(event);
    }), jquery_default()('.dynamic-testimonials-navigation-item .icon-arrow-left').on('click.testimonials', event => {
      event.preventDefault();
      this.navigate(event);
    }), this.$navigationItem.on('mouseout.testimonials', event => jquery_default()(event.currentTarget).blur()), this.$window.on('resize.testimonials', () => {
      const index = this.slideList.children('.active').attr('data-position');
      this.slideHeight(index);
      this.slideWidth();
      if (count > 1) {
    ...
    if (count > 1) {
      ...
      this.slideList.parents('.dynamic-testimonials').addClass('static');
    }
    this.slideHeight(1);
    this.slideWidth();
    if (count > 1) {
    ...
    this.slideList.parents('.dynamic-testimonials').addClass('ready');
  }
  slideHeight(index) {
    this.slideList.height(this.slideList.children(`div[data-position='${index}']`).height());
  }
  slideWidth() {
    this.slideList.width(this.slideList.parent().width() * this.slideList.children().length);
    this.slideList.children().width(this.slideList.parent().width());
  }
  slideAnim(index) {
    this.slideList.css({
  ...
  paginate() {
    ...
    let index = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (jquery_default()(event.currentTarget).hasClass('active') && !index) {
      return;
    }
    if (event && !index) {
      index = jquery_default()(event.currentTarget).attr('data-position');
    }
  ...
  navigate(event) {
    let index;
    event.preventDefault();
    const $navigation = jquery_default()(event.currentTarget).parent('div');
  ...
  onSectionBlockSelect(block) {
    const index = jquery_default()(block.el).index();
    this.paginate(false, index);
  }
  onSectionUnload() {
    this.events.forEach($el => $el.off('.testimonials'));
  }
}
;// CONCATENATED MODULE: ./node_modules/@pixelunion/pxs-announcement-bar/dist/index.es.js

/*!
 * @pixelunion/pxs-announcement-bar v2.0.0
 * (c) 2019 Pixel Union
 */
...
var pxs_announcement_bar_dist_index_es_classCallCheck = function (instance, Constructor) {
  if (!(instance instanceof Constructor)) {
    throw new TypeError("Cannot call a class as a function");
  }
};
...
var PxsAnnouncementBar = function () {
  function PxsAnnouncementBar(section) {
    pxs_announcement_bar_dist_index_es_classCallCheck(this, PxsAnnouncementBar);
  ...
  dist_index_es_createClass(PxsAnnouncementBar, [{
    key: 'onSectionUnload',
    value: function onSectionUnload() {
    ...
    value: function _onScroll() {
      ...
      if (!this.scrolledPast && scrolledPast) {
        ...
        index_es_trigger(this.el, 'pxs-ab-hide');
      }

      // Trigger a show announcement bar is back in view
      if (this.scrolledPast && !scrolledPast) {
        this.scrolledPast = false;
        index_es_trigger(this.el, 'pxs-ab-show');
      }
    }
  }]);
...
/* harmony default export */ const pxs_announcement_bar_dist_index_es = (PxsAnnouncementBar);

;// CONCATENATED MODULE: ./source/scripts/sections/StaticAnnouncementBar.js
...
class AnnouncementBar {
  constructor(instance) {
    ...
    this.el = instance.el;
    this.announcementBar = new pxs_announcement_bar_dist_index_es(instance);
  ...
  _checkAnnouncementBar() {
    ...
    this.$document.trigger('checkAnnouncementBar', {
...
class StaticCart {
  constructor(section) {
    ...
    script_default()([$scripts.data('shopify-api-url'), $scripts.data('shopify-common')], () => {
      script_default()($scripts.data('shopify-countries'), () => {
        script_default()($scripts.data('shopify-cart'), () => {
          this._bindEvents();
          Shopify.onError = XMLHttpRequest => {
            this.handleErrors(XMLHttpRequest);
          };
  ...
  _bindEvents() {
    this.events.push(jquery_default()('.cart-instructions textarea', this.$el).on('change.cart', () => this.saveSpecialInstructions()));
    this.events.push(jquery_default()('[data-calculate-shipping-button]', this.$el).on('click.cart', () => this.calculateShipping()));
    if (this.cartForm) {
  ...
  saveSpecialInstructions() {
    ...
    Shopify.updateCartNote(newNote, cart => {
  ...
  _getItemQuantity(key) {
    return parseInt(this.cartForm.querySelector(`[data-cart-item-key="${key}"] [data-cart-product-quantity]`).value, 10);
  }
  ...
  _setItemQuantity(key, quantity) {
    this.cartForm.querySelector(`[data-cart-item-key="${key}"] [data-cart-product-quantity]`).value = quantity;
  }
  _handleClick(event) {
    ...
    const oldQuantity = this._getItemQuantity(key);
    let newQuantity;
    switch (cartAction) {
      case 'increment':
  ...
  _updateItem(key, quantity) {
    ...
    this.updateTimeout = setTimeout(() => {
      ...
      this.cartForm.querySelectorAll('[data-cart-item-warning-key]').forEach(warning => warning.innerHTML = '');
      jquery_default().ajax({
        type: 'POST',
  ...
  _updateCart(thisTimeoutId) {
    shopify_asyncview_dist_index_es.load(window.Theme.routes.cart_url, {
      ...
      Shopify.getCart(cart => jquery_default()('.header-cart-count').text(cart.item_count));
      ...
      this._generateQuantityWarnings();
    }).catch(() => window.location.reload());
  }

  // Generates a warning for a line item when trying to add more quantity than what's available in inventory
  _generateQuantityWarnings() {
    const {
      cart,
      key,
      requestedQuantity
    ...
    const {
      message
    } = this.context.cart.cartQuantityError;
    let quantityWarningText;
    const item = cart.items.find(item => item.key === key);
    if (item && item.quantity < requestedQuantity) {
      const quantityWarning = this.cartForm.querySelector(`[data-cart-item-warning-key="${key}"]`);
      quantityWarningText = message.replace('** quantity **', item.quantity).replace('** title **', item.title);
      quantityWarning.innerHTML = quantityWarningText;
  ...
  shippingCalculator() {
    ...
    const selectableOptions = this.$shippingCalculator.find('select');
    setTimeout(() => {
      selectableOptions.map((index, el) => {
        this.updateShippingLabel(el);
      });
    }, 500);
    jquery_default()('.cart-shipping-calculator select').change(() => {
      selectableOptions.map(select => this.updateShippingLabel(select));
    });
  }
  calculateShipping() {
    ...
    jquery_default().ajax(`${window.Theme.routes.cart_url}/shipping_rates.json?${queryString}`, {
      ...
      if (rates.length > 1) {
        ...
        responseText = this.context.shipping.multiRates.replace('** address **', address).replace('** number_of_rates **', rates.length).replace('** rate **', `<span class='money'>${firstRate}</span>`);
        ratesFeedback.html(responseText);
      } else if (rates.length === 1) {
        responseText = this.context.shipping.oneRate.replace('** address **', address);
        ratesFeedback.html(responseText);
      } else {
        ratesFeedback.html(this.context.shipping.noRates);
      }
      rates.forEach(rate => {
        const price = Shopify.Cart.ShippingCalculator.formatRate(rate.price);
        const rateValues = this.context.shipping.rateValues.replace('** rate_title **', rate.name).replace('** rate **', `<span class='money'>${price}</span>`);
        jquery_default()('.shipping-rates').append(`<li>${rateValues}</li>`);
      });
      jquery_default()('[data-calculate-shipping-button]').val(this.context.shipping.submitButton);
    });
  }
  updateShippingLabel(value) {
    if (value) {
      const select = jquery_default()(value);
  ...
  handleErrors(errorMessage) {
    if (errorMessage.zip || errorMessage.country) {
      ...
      jquery_default()('.cart-shipping-calculator-response').html(`<p>${errorText}</p>`);
    } else {
...
class StaticCollection {
  constructor(section) {
    ...
    this.facetedFilters = null;
    if (this.el.querySelector('[data-faceted-filter]')) {
...
class Navigation {
  constructor(instance) {
    ...
    this.primaryLinks = this.$el[0].querySelectorAll('[data-primary-link]');
    this.secondaryLinks = this.$el[0].querySelectorAll('[data-secondary-link]');
    jquery_default()('[data-dropdown-toggle]').on('click.navigation', event => this.toggleNavigation(event));
    jquery_default()('[data-dropdown-toggle]').on('keydown.navigation', event => this.toggleNavigationByKey(event));

    // Click

    this.$el.on('click.navigation', () => {
      if (!jquery_default()(event.target).closest('.navigation').length) {
        jquery_default()('.navigation .open').removeClass('open');
      }
    });
    this.$el.on('click.navigation', '.has-dropdown > .header-navigation-link', e => {
      const $target = e.currentTarget;
      this.setMenuLink($target, e);
    });

    // Touch

    this.$document.on('touchend', () => {
      this.closePrimaryMenus();
      this.closeSecondaryMenus();
    });

    // event handler for those dropdown items WITH submenus (uses <details>), preventDefault 
    this.$el.on('touchend.navigation', '.has-dropdown > details.header-navigation-link', e => {
      const $target = e.currentTarget;
      $target.focus();
      e.preventDefault();
      this.setMenuLink($target, e);
      this.openMenu($target, e);
    });

    // for those dropdown items WITHOUT submenus, target the link directly
    this.$el.on('touchend.navigation', '.has-dropdown a.header-navigation-link', e => {
      const $target = e.currentTarget;
      $target.focus();
      this.setMenuLink($target, e);
      this.openMenu($target, e);
    ...
    this.$el.on('focus.navigation', '.has-dropdown > .header-navigation-link', e => {
      const $target = jquery_default()(e.currentTarget).parent('li');
      this.closeTierMenus($target);
    });

    // Mouseover

    this.$el.on('mouseover.navigation', '.has-dropdown > .header-navigation-link', e => {
      const $target = e.currentTarget;
      this.closePrimaryMenus();
      this.openMenu($target, e);
    });
    ...
    this.$el.on('mouseleave.navigation', '.header-navigation-list', () => {
      this.closePrimaryMenus();
    });
    this.$el.on('mouseleave.navigation', '.has-dropdown > .secondary-link', () => {
      this.closeSecondaryMenus();
    });
  }
  setMenuLink($target, e) {
    e.stopPropagation();
    const clickedParent = $target.classList.contains('primary-link') || $target.classList.contains('secondary-link');
    // When the dropdown menu is open, redirect parent links
    if (clickedParent && $target.open === true && $target.getAttribute('href')) {
  ...
  closePrimaryMenus() {
    this.primaryLinks.forEach(primaryLinks => {
      primaryLinks.open = false;
    });
  }
  closeSecondaryMenus() {
    this.secondaryLinks.forEach(secondaryLinks => {
      secondaryLinks.open = false;
    });
  }
  closeSiblingMenus($targetLi) {
    const $siblingMenuParents = $targetLi.siblings('.open');
    this.closeMenus($siblingMenuParents);
  }
  closeTierMenus($target) {
    const $openMenus = $target.parent().find('.open');
    if (!$openMenus.length) {
      return;
    }
    this.closeMenus($openMenus);
  }
  closeMenus($menusParents) {
    $menusParents.each((index, menuParent) => {
      const $menuParent = jquery_default()(menuParent);
      const $toggle = $menuParent.closest(this.dropdownToggle);
      $menuParent.removeClass('open');
      this.toggleMenuButton($toggle, false);
    });
  }

  // Toggle

  toggleMenuButton($button, state) {
    $button.attr('aria-expanded', state);
  }
  toggleNavigationByKey(event) {
    if (event.keyCode !== 13) {
      return;
    }
  ...
  toggleNavigation(event) {
    const $toggle = jquery_default()(event.currentTarget);
    const $parent = $toggle.parent('.has-dropdown');
    const menuIsOpen = $parent.hasClass('open');
  ...
  unload() {
    jquery_default()('[data-dropdown-toggle]').off('.navigation');
    this.$el.off('.navigation');
  }
}
;// CONCATENATED MODULE: ./source/scripts/components/Goal.js
...
class Goal {
  constructor(_ref) {
    ...
    // Product goal can be a dynamic section which uses the global theme settings
    // or a block within the product page or featured product section and
    // has its own block settings
    this.$selector = this.context === 'block' ? this.$el : jquery_default()('[data-goal-countdown]');
    this.endDateTime = this.$selector.data('end-time');

    // TODO: Do better checking or parsing on passed value to make sure its a proper date
    if (this.$selector.length && this.getRemainingTime().total >= 0) {
      this.countDownTimer();
    }
    if (this.context !== 'block') {
      jquery_default()('body').addClass('has-goal-expired');
    }
  }
  getRemainingTime() {
    const _second = 1000;
    ...
    const _day = _hour * 24;
    const t = Date.parse(this.endDateTime) - Date.parse(new Date());
    const seconds = Math.floor(t / _second % 60);
    const minutes = Math.floor(t / _minute % 60);
    const hours = Math.floor(t / _hour % 24);
    const days = Math.floor(t / _day);
    return {
      total: t,
      days,
      hours,
  ...
  countDownTimer() {
    ...
    if (this.context === 'block') {
      this.$productViewDays = this.$el.find('.product-goal-info-days');
      this.$productViewDaysWrapper = this.$el.find('.product-goal-info-remaining');
    }
    this.runTimer();
    if (this.context !== 'block') {
      ...
      setTimeout(() => {
        ...
        this.$document.trigger('checkHeaderOffsets');
      }, 100);
    }
  }
  runTimer() {
    ...
    if (this.context === 'block' && this.$productViewDays.length) {
      if (timeRemaining.total > 0 && timeRemaining.days === 0) {
        this.$productViewDaysWrapper.html(this.lastDayText);
      }
      this.$productViewDays.html(timeRemaining.days);
    }
  }
}
...
/* harmony default export */ const ThemeUtils = ({
  ...
  scrollTarget($el) {
    ...
    const $stickyHeader = jquery_default()('[data-sticky-header]');
  ...
  externalLinks($el) {
    const anchors = $el.find('a[href^="http"]').filter((i, el) => el.href.indexOf(location.hostname) === -1);
...
class StaticHeader {
  constructor(instance) {
    ...
    this.windowWidth = ThemeUtils.windowWidth();
    ...
    this.slideShow = '[data-static-slideshow]';
    this.$slideShow = jquery_default()(this.slideShow);
    this.$document.on('shopify:section:select', () => this.sectionChanges(null)).on('shopify:section:deselect', () => this.sectionChanges(null)).on('click.search', '[data-search-toggle]', () => this._openSearch()).on('blur.search', '.header-search-input', () => this._closeSearch()).on('click.drawer', '[data-drawer-toggle]', event => this._toggleDrawerNav(event)).on('calculateHeaderWidths', () => this._calculateHeaderWidths()).on('toggleStickyHeader', (event, $slideshow) => this.stickyHeader($slideshow)).on('toggleSlideShowHeader', (event, $slideshow) => this.slideShowHeader($slideshow)).on('checkAnnouncementBar', (event, data) => this._checkAnnouncementBar(data)).on('checkHeaderOffsets', () => this._headerOffsets());
    this.window.on('resize.header', () => this._resizeEvents());

    // This can't be debounced as it creates a visual delay
    this.window.on('scroll.header', () => this._headerOffsets());
  ...
  onSectionUnload() {
    ...
    // Remove Goal logic when section unloads
    if (this.goal != null) {
      this.goal.remove();
    }
  ...
  _resizeEvents() {
    // Alternate logo has potential to be a different width
    this._headerOffsets();
    const currentWindowWidth = this.themeUtils.windowWidth();

    // Only close navigation if width has changed
    if (this.$document.hasClass('showing-drawer') && currentWindowWidth !== this.windowWidth) {
      this.windowWidth = this.themeUtils.windowWidth();
      this._toggleDrawerNav();
    }
  }
  ...
  */
  sectionChanges($slideshow) {
    this.stickyHeader($slideshow);
    this.slideShowHeader($slideshow);
  }
  stickyHeader($slideShow) {
    if ($slideShow == null) {
      $slideShow = jquery_default()(this.slideShow);
    }
    this.$document.toggleClass('has-sticky-header', this.isHeaderSticky);
    this._headerOffsets($slideShow);
  }
  _headerOffsets($slideShow) {
    if ($slideShow == null) {
      $slideShow = jquery_default()(this.slideShow);
    }
    const isFullWindowSlideshow = $slideShow.attr('data-full-window-slideshow') != null;
    const {
      pageYOffset
    } = window;
    ...
    if (this.announcementBarEl) {
      // Scroll related styles only apply when scrolled past AnnouncementBar
      scrollThreshold = this.announcementBarRect.height;
    }
    const pastThreshold = pageYOffset > scrollThreshold;

    // Adjust the header in situations where it is absolute
    if (this.isHeaderSticky || isFullWindowSlideshow) {
      // When full window slideshow, @$headerContainer is 'top: 0'
      ...
      if (this.isHeaderSticky) {
        // Offset absolute positioning to top of viewport, or below  AnnouncementBar
        ...
        if (!isFullWindowSlideshow) {
          ...
          // If header is sticky, and no announcement bar, no offset
          if (this.themeUtils.windowWidth() < 720) {
            bodyOffset = 0;
          }

          // If there is an announcement bar, offset by this its height
          if (this.themeUtils.windowWidth() < 720 && this.announcementBarEl) {
            bodyOffset = this.announcementBarRect.height;
          }
        }
        if (this.announcementBarEl && !pastThreshold) {
          const bottomEdge = scrollThreshold - pageYOffset;
        ...
        if (this.$headerGoal.length && this.$headerGoal.hasClass('active')) {
          if (this.themeUtils.windowWidth() < 720) {
            ...
            const goalHeight = this.$headerGoal.height();
  ...
  slideShowHeader($slideShow) {
    ...
    if ($slideShow.length && $slideShow.attr('data-full-window-slideshow') != null) {
      ...
      this.$document.removeClass('has-slideshow-full-window');
    }
  }
  _calculateHeaderWidths() {
    let brandingWidth = this.$el.find('.branding').outerWidth(true);
    const toolsWidth = this.$el.find('.header-tools').outerWidth(true);
    ...
    */
    ...
    if (this.$headerLogo.length) {
      const logoWidths = [];
      this.$headerLogo.each((index, logo) => {
        logoWidths.push(jquery_default()(logo).width());
      });
      brandingWidth = Math.max.apply(Math, logoWidths);
    }
    this.headerContentWidth = Math.ceil(brandingWidth) + Math.ceil(toolsWidth) + gutterWidth;
  }
  _toggleDrawerNav() {
    ...
    const isDrawerOpen = this.$document.hasClass('showing-drawer');
    this.$document.toggleClass('showing-drawer', !isDrawerOpen);
    if (isDrawerOpen) {
      // Drawer is going to close, store existing top
      const top = this.$headerContainer.data('old-top');
      if (top) {
    ...
    this.$headerContainer.one('transitionend', () => {
      this.$document.toggleClass('drawer-visible', !isDrawerOpen);
      this.$drawerToggle.attr('aria-expanded', !isDrawerOpen);
    });
  }
  _openSearch() {
    ...
    this.$el.find('.header-search-wrapper').on('keyup.search', event => {
      if (event.keyCode === 27) {
        this._closeSearch();
      }
    });
  ...
  _closeSearch() {
    this.$el.find('.header-search-wrapper').removeClass('active').off('keyup.search');
  }
  _checkAnnouncementBar(data) {
    const {
      el
    } = data;

    // Refresh globals when AnnouncementBar is updated
    this.announcementBarEl = el;
    this.$announcementBarEl = jquery_default()(this.announcementBarEl);
    this.announcementBarRect = this.announcementBarEl != null ? this.announcementBarEl.getBoundingClientRect() : undefined;
    this._headerOffsets();
  }
}
...
class StaticPassword {
  constructor(section) {
    this.$el = jquery_default()(section.el);
    this.themeUtils = ThemeUtils;
    this.events = [jquery_default()('.password-entry', this.$el).on('click.password', () => this.togglePasswordForm()), jquery_default()('.password-embedded-newsletter-form', this.$el).on('submit.password', () => this.toggleNewsletterSuccess()), jquery_default()(window).on('load resize.password', this.themeUtils.debounce(this.setContentHeight, 100))];
    ...
    if (jquery_default()('[data-password-form-inner]').hasClass('has-errors')) {
      this.togglePasswordForm();
    }
  }
  toggleNewsletterSuccess(event) {
    if (this.emailRegEx.test(jquery_default()(this.newsletterInput).val())) {
      jquery_default()('.password-embedded-newsletter-form').addClass('hidden');
      jquery_default()('.form-success.hidden').removeClass('hidden');
    } else {
      event.preventDefault();
    }
  }
  ...
  setContentHeight() {
    const footer = jquery_default()('.footer-wrapper', this.$el);
    const windowHeight = jquery_default()(window).height();
    const headerHeight = jquery_default()('.main-header-wrapper', this.$el).outerHeight(true);
    const footerHeight = footer.outerHeight(true);
    const contentHeight = jquery_default()('.main-content', this.$el).outerHeight(true);
...
class StaticProductRecommendations {
  ...
  _loadRecommendations() {
    shopify_asyncview_dist_index_es.load(this.recommendUrl, {
      ...
      if (productItems.length && window.Shopify && Shopify.PaymentButton) {
...
class StaticSlideshow {
  constructor(_ref) {
    ...
    // Elements
    this.el = el;
    this.slideshowWrapper = this.el.querySelector('.static-slideshow-wrapper');
    this.slideshowContent = this.el.querySelector('[data-slideshow-content]');
    this.slides = this.el.querySelectorAll('[data-slide]');
    this.headerNavigation = document.querySelector('[data-main-header]');
    this.slideNavigation = this.el.querySelectorAll('.static-slideshow-navigation');
    this.slidePagination = this.el.querySelector('.static-slideshow-pagination');
    this.activeSlide = null;

    // Variables
    this.fullWindowWidth = window.innerWidth;
    this.fullWindowHeight = window.innerHeight;
    this.slideshowType = data.type;

    // Settings
    this.autoplaySpeed = data.autoplay_speed;
    this.slideshowLayout = data.layout;
    this.autoplayEnabled = data.enable_autoplay;

    // Event listeners
    this.events = [];
    if (this.slideNavigation.length) {
      this._registerEvent(this.slideNavigation[0], 'click', e => {
        e.preventDefault();
        this._previousSlide();
      });
      this._registerEvent(this.slideNavigation[1], 'click', e => {
        e.preventDefault();
        this._nextSlide();
      });
      this._registerEvent(this.slidePagination, 'click', e => {
        e.preventDefault();
        const {
          parentNode
        } = e.target;
        if (parentNode.classList.contains('pagination-list-item')) {
          this._specificSlide(parentNode);
        }
      });
    }
    this._registerEvent(this.el, 'mouseenter', () => this._pauseLoop());
    this._registerEvent(this.el, 'mouseleave', () => this._startLoop());
    this._registerEvent(this.el, 'rimg:load', () => this._setActiveSlideHeight());
    this._registerEvent(window, 'resize', () => {
      // This is to prevent resize on scroll of Android and iOS when the address bar is hidden.
      if (window.innerWidth === this.fullWindowWidth) return;
      this.fullWindowWidth = window.innerWidth;
      this.fullWindowHeight = window.innerHeight;
      this._setActiveSlideHeight();
    });

    // Initializers
    this._setupSlides();

    // Adds helper class to header when slideshow set to 'full window'
    if (this.slideshowType === 'static') {
      if (this.slideshowLayout === 'full-window') {
  ...
  _setupSlides() {
    ...
    this._setSlideshowClasses();
    this._setActiveSlideHeight();
    this.el.classList.add('slides-ready');
    this._startLoop();
  }
  _setActiveSlideHeight() {
    const slideText = this.activeSlide.querySelector('.slide-text');
    const slideImage = this.activeSlide.querySelector('.slide-image');
    const slidePadding = this.slidePagination ? this.slidePagination.getBoundingClientRect().height * 2 : 60;
    const textHeight = slideText ? slideText.getBoundingClientRect().height + slidePadding : 0;
    const imageHeight = slideImage.getBoundingClientRect().height;
    if (!imageHeight) return;
    let slideHeight;
    if (this.slideshowLayout === 'full-window') {
      const headerHeight = this.headerNavigation.getBoundingClientRect().height;

      // If there is too much text, the height of the slide should be more than the full
      // height of the window.
      if (textHeight > window.innerHeight - headerHeight) {
        slideHeight = window.innerWidth > 720 ? textHeight + headerHeight * 2 : textHeight;
      } else {
        slideHeight = window.innerWidth > 720 ? this.fullWindowHeight : this.fullWindowHeight - this.headerNavigation.getBoundingClientRect().height;
      }
      const announcementBarEl = document.querySelector('.pxs-announcement-bar');

      // If is full window, subtract announcement bar height
      if (announcementBarEl) {
        slideHeight -= announcementBarEl.getBoundingClientRect().height;
      }
      this.slideshowWrapper.style.height = `${slideHeight}px`;

      // Update height of all slides since they should all be the same height.
      // This is to ensure all lazy loading of images crop the same.
      for (let i = 0; i < this.slides.length; i++) {
        this.slides[i].style.height = `${slideHeight}px`;
      }
    } else {
      // Minimum height is height of text, or image, which ever is larger
      slideHeight = Math.max(textHeight, imageHeight);
      this.slideshowWrapper.style.height = `${slideHeight}px`;
      this.activeSlide.style.height = `${slideHeight}px`;
    }
  }
  _setSlideshowClasses() {
    const slideTextStyle = this.activeSlide.getAttribute('data-slide-text');
    if (this.slideshowType === 'static') {
      document.body.classList.remove('slide-color-light');
      document.body.classList.remove('slide-color-dark');
      document.body.classList.add(`slide-color-${slideTextStyle}`);
    }
    this.slideshowWrapper.classList.remove('slide-color-light');
    this.slideshowWrapper.classList.remove('slide-color-dark');
    this.slideshowWrapper.classList.add(`slide-color-${slideTextStyle}`);
  }
  _specificSlide(target) {
    if (this.sliding) return;
    if (!target.classList.contains('active')) {
      const nextSlideID = target.getAttribute('data-slide-id');
      this._showNewSlide('next', nextSlideID);
    }
  }
  _previousSlide() {
    if (this.sliding) return;
    this._showNewSlide('prev');
  }
  _nextSlide() {
    if (this.sliding) return;
    this._showNewSlide('next');
  }
  _getIndexOfSlide(slide) {
    for (let i = 0; i < this.slides.length; i++) {
      if (this.slides[i] === slide) return i;
    }
    return -1;
  }
  _showNewSlide(type, specificSlideID) {
    this.sliding = true;
    if (this.slides.length === 1 || this.activeSlide.id === specificSlideID) {
      this.sliding = false;
      return;
    }
    const direction = type === 'next' ? 'left' : 'right';
    const activeSlideIndex = this._getIndexOfSlide(this.activeSlide);
    if (specificSlideID) {
      this.nextSlide = this.el.querySelector(`#${specificSlideID}`);
    } else if (type === 'next') {
      this.nextSlide = activeSlideIndex + 1 >= this.slides.length ? this.slides[0] : this.slides[activeSlideIndex + 1];
    } else {
      this.nextSlide = activeSlideIndex - 1 < 0 ? this.slides[this.slides.length - 1] : this.slides[activeSlideIndex - 1];
    }
    if (this.activeSlide.id === this.nextSlide.id) return;
    this.nextSlide.classList.add(type);

    // force reflow for slide effect
    this.nextSlide.offsetWidth; // eslint-disable-line

    this.activeSlide.classList.add(direction);
    this.nextSlide.classList.add(direction);
    this.previousSlide = this.activeSlide;
    this._slideListener = this._registerEvent(this.nextSlide, 'transitionend', () => this._slideTransitionEnd()).listener;
    this.activeSlide = this.nextSlide;
    this._setActiveSlideHeight();
    this._setSlideshowClasses();
    this._updateSlidePagination();
  }
  _slideTransitionEnd() {
    this.nextSlide.classList.remove('next');
    this.nextSlide.classList.remove('prev');
    this.nextSlide.classList.remove('right');
    this.nextSlide.classList.remove('left');
    this.nextSlide.classList.add('active');
    this.previousSlide.classList.remove('active');
    this.previousSlide.classList.remove('right');
    this.previousSlide.classList.remove('left');
    this.previousSlide = null;
    this._unregisterEvent(this.nextSlide, 'transitionend', this._slideListener);
    this.nextSlide = null;
    this.sliding = false;
  }
  _startLoop() {
    if (!this.autoplayEnabled || this.slidesLocked) return;
    this.autoplayInterval = setInterval(() => this._nextSlide(), this.autoplaySpeed);
  }
  _pauseLoop() {
    if (this.autoplayInterval == null) return;
    clearInterval(this.autoplayInterval);
  }
  _updateSlidePagination() {
    const index = this._getIndexOfSlide(this.activeSlide);
    this.slidePagination.querySelector('.active').classList.remove('active');
    this.slidePagination.querySelectorAll('.pagination-list-item')[index].classList.add('active');
  }
  _lockSlide(slideID) {
    this.slidesLocked = true;
    this._pauseLoop();
    this._showNewSlide('next', slideID);
  }
  _unlockSlide() {
  ...
  onSectionBlockSelect(block) {
    this._lockSlide(block.el.id);
  }
  onSectionBlockDeselect() {
    this._unlockSlide();
  }
  onSectionUnload() {
    this.events.forEach(_ref2 => {
      let {
        el,
        event,
        listener
  ...
  // This function is for event handling. The hope is that this would be
  // moved into the SectionsManager class so it would be available to all
  // sections that we create. We could also have an unregisterEvent.
...
class Accordion {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    jquery_default()('[data-accordion-trigger]', this.$el).on('click.accordion', event => this._toggle(event));
  ...
  _toggle(event) {
    ...
    const trigger = $target.attr('data-accordion-trigger');
    const $content = jquery_default()(`[data-accordion-content=${trigger}]`);
  ...
  _closeAll() {
    jquery_default()(this.accordionTrigger).each((index, accordion) => {
      const $target = jquery_default()(accordion);
      if ($target.hasClass(this.activeAccordion)) {
        $target.trigger('click');
      }
    });
  }
  _open($target, $content) {
    $content.slideDown({
      duration: 400,
      start: () => $target.addClass(this.activeAccordion)
    });
  }
  _close($target, $content) {
    $content.slideUp({
      duration: 400,
      start: () => $target.removeClass(this.activeAccordion)
    });
  }
  onSectionBlockSelect(block) {
    const $container = jquery_default()(block.el);
    const $target = $container.find(this.accordionTrigger);
    if (!$target.hasClass(this.activeAccordion)) {
      $target.trigger('click');
    }
  }
  unload() {
    jquery_default()('[data-accordion-trigger]', this.$el).off('.accordion');
  }
}
...
function dist_index_es_unsupportedIterableToArray(o, minLen) {
  if (!o) return;
  if (typeof o === "string") return dist_index_es_arrayLikeToArray(o, minLen);
  var n = Object.prototype.toString.call(o).slice(8, -1);
  if (n === "Object" && o.constructor) n = o.constructor.name;
  if (n === "Map" || n === "Set") return Array.from(n);
  if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return dist_index_es_arrayLikeToArray(o, minLen);
}
...
var evEmitter = dist_index_es_createCommonjsModule(function (module) {
/**
 * EvEmitter v1.1.0
 * Lil' event emitter
 * MIT License
...
var getSize = dist_index_es_createCommonjsModule(function (module) {
/*!
 * getSize v2.0.3
 * measure size of elements
 * MIT license
...
var matchesSelector = dist_index_es_createCommonjsModule(function (module) {
/**
 * matchesSelector v2.0.2
 * matchesSelector( element, '.selector' )
 * MIT license
...
var utils = dist_index_es_createCommonjsModule(function (module) {
/**
 * Fizzy UI utils v2.0.7
 * MIT license
 */
...
var cell = dist_index_es_createCommonjsModule(function (module) {
// Flickity.Cell
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var slide = dist_index_es_createCommonjsModule(function (module) {
// slide
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var animate = dist_index_es_createCommonjsModule(function (module) {
// animate
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var flickity = dist_index_es_createCommonjsModule(function (module) {
// Flickity main
...
var unipointer = dist_index_es_createCommonjsModule(function (module) {
/*!
 * Unipointer v2.4.0
 * base class for doing one thing with pointer event
 * MIT license
...
var unidragger = dist_index_es_createCommonjsModule(function (module) {
/*!
 * Unidragger v2.4.0
 * Draggable base class
 * MIT license
...
var drag = dist_index_es_createCommonjsModule(function (module) {
// drag
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var prevNextButton = dist_index_es_createCommonjsModule(function (module) {
// prev/next buttons
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var pageDots = dist_index_es_createCommonjsModule(function (module) {
// page dots
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var player = dist_index_es_createCommonjsModule(function (module) {
// player & autoPlay
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var addRemoveCell = dist_index_es_createCommonjsModule(function (module) {
// add, remove cell
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var lazyload = dist_index_es_createCommonjsModule(function (module) {
// lazyload
( function( window, factory ) {
  // universal module definition
  if (  module.exports ) {
...
var js = dist_index_es_createCommonjsModule(function (module) {
/*!
 * Flickity v2.3.0
 * Touch, responsive, flickable carousels
 *
...
function shopify_surface_pick_up_dist_index_es_objectSpread2(target) {
  for (var i = 1; i < arguments.length; i++) {
    var source = arguments[i] != null ? arguments[i] : {};
...
var SurfacePickUp = /*#__PURE__*/function () {
  function SurfacePickUp(el, options) {
    ...
    this.el = el;
    this.options = shopify_surface_pick_up_dist_index_es_objectSpread2({
...
;// CONCATENATED MODULE: ./source/scripts/utils/LinkedOptions.js
    ...
    if (this.options.type === 'radio') {
      switch (optionIndex) {
        ...
        case 1:
          ...
          break;
        case 2:
          const option1 = this.options.$selector.filter('[data-option-index=0]').filter(':checked').val();
          const option2 = this.options.$selector.filter('[data-option-index=1]').filter(':checked').val();
          key = `${option1} / ${option2}`;
  ...
  _mapVariants(product) {
    ...
    product.variants.forEach(variant => {
      if (variant.available) {
        ...
        optionsMap.root = this.themeUtils.unique(optionsMap.root);
        if (product.options.length > 1) {
          key = variant.option1;
          optionsMap[key] = optionsMap[key] || [];
          optionsMap[key].push(variant.option2);
          optionsMap[key] = this.themeUtils.unique(optionsMap[key]);
        }
...
class VariantHelper {
  constructor(options) {
    const defaultOptions = {
      ...
      productSettings: null // Set with JSON containing product message strings and/or display options
    };

    this.themeUtils = ThemeUtils;
    this.callbacks = [];
    this.options = this.themeUtils.extend(defaultOptions, options);
    this.$body = jquery_default()(document.body);
    ...
    // Disabled history state updating in the TE
    ...
    this._bindEvents();
    if (this.options.type === 'select') {
      jquery_default()(this.options.$selector).each((index, element) => {
        this._setSelectLabel(null, jquery_default()(element));
      });
    }
  ...
  _bindEvents() {
    this.options.$selector.on('change', event => this._variantChange(event));
  }
  _setSelectLabel() {
    let event = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    if (!$target) {
      $target = jquery_default()(event.currentTarget);
    }
    const selectedOption = $target.find('option:selected').val();
    $target.prev('[data-select-text]').find('[data-selected-option]').text(selectedOption);
  }
  _getCurrentOptions() {
  ...
  _updatePrice(variant) {
    ...
    const $displayPrice = $priceFields.find('.money:first-child');
    const $originalPrice = $priceFields.find('.original');
    const $amountSaved = $priceFields.find('.saving-result');
    const $priceComparison = $priceFields.find('.product-price-compare');
    if (variant) {
      if (variant.available) {
  ...
  _updateTaxLine(variant) {
    if (!variant) return;
    this.options.$priceFields.each((i, el) => {
      const taxLine = el.querySelector('[data-tax-line]');
      if (!taxLine) return;
      if (variant.taxable) {
        taxLine.classList.remove('product__tax--hidden');
      } else {
        taxLine.classList.add('product__tax--hidden');
      }
    });
  }
  _updateUnitPrice(variant) {
    ...
    this.options.$priceFields.each((i, el) => {
      const variantFields = {
        unitPrice: el.querySelector('[data-unit-price]'),
        totalQuantity: el.querySelector('[data-total-quantity]'),
        unitPriceAmount: el.querySelector('[data-unit-price-amount]'),
        unitPriceMeasure: el.querySelector('[data-unit-price-measure]')
      };
      if (variantFields.unitPrice && variant.unit_price_measurement) {
        variantFields.totalQuantity.innerHTML = `${variant.unit_price_measurement.quantity_value}${variant.unit_price_measurement.quantity_unit}`;
        variantFields.unitPriceAmount.innerHTML = Shopify.formatMoney(variant.unit_price, Theme.moneyFormat);
        if (variant.unit_price_measurement.reference_value === 1) {
          variantFields.unitPriceMeasure.innerHTML = variant.unit_price_measurement.reference_unit;
        } else {
          variantFields.unitPriceMeasure.innerHTML = `${variant.unit_price_measurement.reference_value}${variant.unit_price_measurement.reference_unit}`;
        }
        variantFields.unitPrice.classList.remove('product__unit-price--hidden');
      } else if (variantFields.unitPrice) {
        variantFields.unitPrice.classList.add('product__unit-price--hidden');
      }
    });
  }
  _variantChange(event) {
    ...
    this._updateSmartPaymentButtons(variant);
    this._updateUnitPrice(variant);
    this.callbacks.forEach(callback => callback(variant));
  }
  _updateSmartPaymentButtons(variant) {
    if (variant && variant.available) {
      this.$smartPaymentButtons.slideDown();
    } else {
      this.$smartPaymentButtons.slideUp();
    }
  }
  prepareRemove() {
    this.options.$selector.off('change');
    if (this.linkedOptions) {
      this.linkedOptions.prepareRemove();
    }
  }
}
...
class ProductGallery {
  constructor(el, settings) {
    ...
    if (this.navigation) {
      ...
      this.navigationArrows = this.navigation.querySelectorAll('.product-slideshow-navigation');
      this.onNavigationArrowClick = e => {
        const direction = e.target.dataset.slideshowMovement;
        if (direction === 'next') {
      ...
      this.events.register(this.navigation, 'click', event => {
        if ('productGalleryThumbnail' in event.target.dataset) {
          this.flickity.select(parseInt(event.target.dataset.productGalleryThumbnail, 10));
        }
      });
      this.events.register(this.navigation, 'focusin', event => {
        // Selects thumbnails when tabbing, bringing focused thumbnail into view
        // Note that thumbnails can be selected in the flickity slider
        // without the corresponding media being selected in the main viewport.
        // Main viewport is only updated when thumbnail is selected with a
        // click event, which includes activation with the keyboard.
        if (!('productGalleryThumbnail' in event.target.dataset)) return;
        this.flickityThumbnails.select(parseInt(event.target.dataset.productGalleryThumbnail, 10));
      });
      this.navigationArrows.forEach(arrow => this.events.register(arrow, 'click', this.onNavigationArrowClick));
      cjs/* onChange */.z2(breakpoints => this._onBreakpointChange(breakpoints));
    }
  ...
  _onModelLibraryLoad() {
    ...
    modelFigures.forEach(figure => {
      ...
      const modelEl = figure.querySelector('model-viewer');
      this.media.push({
        type: 'model',
        figure,
        player,
  ...
  _onBreakpointChange(_ref) {
    ...
    if (previous.value === 'S' || current.value === 'S') {
      // Desktop and mobile use slightly different config,
      // so we need to re-initialize when switching between them.
      this._disableFlickity();
      this._initFlickity();
    }
  }
  _selectMediaByEl(el) {
    if (this.flickity) {
      this.flickity.select(parseInt(el.dataset.productGalleryFigure, 10));
    }
  }
  _onVideoLibraryLoad() {
  ...
  _onExternalVideoLibraryLoad() {
    ...
    videoFigures.forEach(videoFigure => {
      ...
      player.addEventListener('onStateChange', _ref2 => {
        let {
          data
        } = _ref2;
  ...
  _onMediaSelect(media) {
    ...
    switch (media.type) {
      case 'video':
        ...
        break;
      case 'model':
        if (document.querySelector('html').classList.contains('no-touch') && cjs/* min */.VV('L')) {
          media.play();
        }
        break;
      default:
  ...
  _initFlickity() {
    const flickityGalleryOptions = {
      ...
      wrapAround: true,
      freeScroll: false,
      initialIndex: parseInt(this.selected.figure.dataset.productGalleryFigure, 10),
    ...
    const flickityThumbnailOptions = {
      accessibility: false,
      pageDots: false,
      wrapAround: this._shouldThumbnailsWrap(),
      initialIndex: parseInt(this.selected.figure.dataset.productGalleryFigure, 10),
      prevNextButtons: false,
      cellSelector: '[data-product-gallery-thumbnail]'
    };
    this.flickity = new (js_default())(this.viewport, flickityGalleryOptions);
    this.flickityThumbnails = new (js_default())(this.navigation, flickityThumbnailOptions);
    this.navScroller = this.flickityThumbnails.element;
    this.flickityEvent = this.events.register(this.viewport, 'rimg:load', () => this.flickity.resize());
    this.flickityThumbnailsEvent = this.events.register(this.navigation, 'rimg:load', () => this.flickityThumbnails.resize());
    this.viewport.querySelectorAll('video').forEach(video => this.events.register(video, 'loadeddata', () => {
      if (this.flickity) {
        this.flickity.resize();
      }
    }));

    // Ensures models and video auto-play when swiping through the slideshow
    this.flickity.on('change', index => {
      this._selectMediaByIndex(index);
      if (this.flickityThumbnails.selectedIndex !== index) {
        this.flickityThumbnails.select(index);
      }
    });
    this.flickity.on('settle', () => {
      this.media.filter(m => m.figure === this.selected.figure)[0].focus();
    });
    ...
    this.flickity.on('staticClick', (event, pointer, cellElement) => {
      ...
      if (event.target.className === 'product-gallery--viewinyourspace' || !cellElement) {
        return;
      }
      const figure = cellElement.dataset.mediaType === 'model' ? cellElement : false;
      if (figure) {
        const model = this.media.filter(item => item.figure === figure)[0];
        model.play();
      }
    });
  }
  _shouldThumbnailsWrap() {
    // Mobile = 2 arrows + 3 slides
    // Desktop = 2 arrows + 5 slides
    return cjs/* max */.Fp('S') ? this.navigation.childElementCount > 5 : this.navigation.childElementCount > 7;
  }
  _disableFlickity() {
    if (this.flickity) {
    ...
    if (this.flickityThumbnails) {
      this.flickityThumbnails.destroy();
      this.flickityThumbnails = null;
      this.events.unregister(this.flickityThumbnailsEvent);
    }
  }
}
...
class Modal {
  constructor() {
    ...
    this.modalInnerEl = this.el.querySelector('[data-modal-inner]');
    this.contentsEl = this.el.querySelector('[data-modal-contents]');
    this.closeEl = this.el.querySelector('[data-modal-close]');
    this.el.addEventListener('click', () => this.close());
    this.modalInnerEl.addEventListener('click', e => e.stopPropagation());
  ...
  showModal() {
    ...
    $(this.el).one('trend', () => this.el.classList.add(this.selectors.active));
  }
}
...
class PaymentTerms {
  constructor(el) {
    ...
    this._targets = this._el.querySelectorAll('[data-payment-terms-target]');
    this._inputs = [];

    // Launch uses separate markup for desktop and mobile so we need
    // to copy the terms to both places.
    this._targets.forEach(target => {
      const input = document.createElement('input');
      input.name = 'id';
      input.type = 'hidden';
      target.appendChild(input);
      target.appendChild(this._reference.cloneNode(true));
...
class ProductDetails {
  constructor(section) {
      ...
      this.modal = new Modal({
        ...
        callbacks: {
          onOpen: _ref => {
            ...
            if (this.productJSON) {
              const variant = this.variantHelpers ? this.variantHelpers.getCurrentVariant() : this.productJSON.variants[0];
              modalHeader.innerHTML = `
                  <h2 class="surface-pick-up-modal__header">
                    ${this.productJSON.title}
                  </h2>
                  ${this.variantHelpers ? `<span class="surface-pick-up-modal__subtitle">${variant.title}</span>` : ''}
                `;
            }
    ...
    this.productGoal = new Goal({
      el: this.$el.find('[data-product-goal-block]'),
      context: 'block'
    });
    ...
    this.$el.on('click', '.number-input-nav-item', event => this.amount(event));
    this.$addToCartButton.on('click', event => this.addToCart(event));
    if (this.$variantDropdowns.length) {
      this.setupVariants();
    } else {
      this.surfacePickUp.load(this.productJSON.variants[0].id); // Single variant product
    }
  }
  ...
  setupVariants() {
    ...
    this.variantHelpers.onVariantChange(variant => {
      if (!variant) return;
      const id = {
        variant
      };
      this.surfacePickUp.load(id);
      this.paymentTerms.update(id);
    });
    this.$variantDropdowns.trigger('change');
  }
  addToCart(e) {
    e.preventDefault();
    const isPasswordPage = document.querySelector('.template-password');
    if (this.processing) {
      return;
    }
    this.processing = true;
    this.$el.find('.product-wrap').addClass('loading');
    const quantity = this.$el.find("input[name='quantity']").val();
    if (quantity === '' || quantity === '0' || isPasswordPage) {
      setTimeout(() => {
        this.$el.find("input[name='quantity']").addClass('error');
        if (isPasswordPage) {
          this.$productMessage.text(this.productSettings.errorText);
        } else {
          this.$productMessage.text(this.productSettings.setQuantityText);
        }
        this.$el.find('.product-wrap').removeClass('loading added-success').addClass('added-error');
        this.processing = false;
      }, 500);
    } else {
      jquery_default().ajax({
        ...
        data: this.$productForm.serialize(),
        dataType: 'json'
      })
      // On success
      .done(cartItem => {
        setTimeout(() => {
          Shopify.getCart(cart => jquery_default()('.header-cart-count').text(cart.item_count));
          const successMessage = this.productSettings.successMessage.replace('** product **', `<em>${cartItem.title}</em>`);
          this.$productMessage.html(successMessage);
          this.$el.find("input[name='quantity']").removeClass('error');
          this.$el.find('.product-wrap').removeClass('loading added-error').addClass('added-success');
          jquery_default()('.header-cart-count').addClass('active');
          this.$addToCartButton.val(this.productSettings.addToCartText);
          this.processing = false;

          // Enable cart redirect if setting enabled
          if (this.productSettings.cartRedirect || document.querySelector('.template-cart')) {
            window.location.href = window.Theme.routes.cart_url;
          }
        }, 1000);
      })
      // On failure
      .fail(response => this.handleErrors(response));
    }
  }
  handleErrors(errors) {
    ...
    if ((errorMessage.description ? errorMessage.description.indexOf(productTitle) : -1) > -1) {
      errorDescription = errorDescription.replace(productTitle, `<em>${productTitle}</em>`);
    }
    if (errorMessage.message === 'Cart Error') {
      setTimeout(() => {
        this.$el.find("input[name='quantity']").removeClass('error');
        this.$productMessage.html(errorDescription);
        this.$el.find('.product-wrap').removeClass('loading added-success').addClass('added-error');
        this.$addToCartButton.val(this.productSettings.addToCartText);
        this.processing = false;
      }, 1000);
  ...
  amount(e) {
    const input = jquery_default()(e.currentTarget).parents('.number-input-wrapper').find('input');
    const result = parseFloat(input.val());
    if (jquery_default()(e.currentTarget).hasClass('icon-plus')) {
      if (result + 1 != 0) {
        input.val(result + 1);
      }
    }
    if (jquery_default()(e.currentTarget).hasClass('icon-minus') && result > 0) {
      if (result - 1 != 0) {
        input.val(result - 1);
      }
    }
  ...
  unload() {
    this.$el.off('submit', '.product-form').off('click', '.number-input-nav-item');
...
class StaticProduct {
  constructor(section) {
    ...
    script_default()(jquery_default()('[data-scripts]').data('shopify-api-url'), () => {
      this.productDetails = new ProductDetails(section);
      if (this.data.complementary_product_limit) {
        this.complementaryProducts = new pxs_complementary_products_dist_index_es({
          ...
          limit: this.data.complementary_product_limit
        });
      }
    });
  }
...
class RTE {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.$select = jquery_default()('.select-wrapper select', this.$el);
    this.$select.on('change', () => this.updateOption());
    ...
    fitvids_default()(this.$el);
    this.setupTabs();
  }
  setupTabs() {
    this.tabs = new (dist_default())(document.querySelector('.rte'), {
      layout: 'tabs'
    });
  }
  updateOption(e, selector) {
...
class Select {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.$el.on('change.select', () => this.updateSelectText());
    this.$el.on('blur.select', () => this.blurSelect());
    this.$el.on('focus.select', () => this.focusSelect());
  ...
  blurSelect() {
    this.$el.parent('.select-wrapper').toggleClass('active', false);
  }
  focusSelect() {
    this.$el.parent('.select-wrapper').toggleClass('active', true);
  }
  updateSelectText() {
    const newOption = this.$el[0].querySelector('option:checked');
    const optionText = newOption ? newOption.innerHTML : '';
    if (this.$el.siblings('.selected-option').length) {
      this.$el.siblings('.selected-option').text(optionText);
    } else {
      this.$el.siblings('.selected-text').text(optionText);
    }
  }
  unload() {
    this.$el.off('.select');
...
class Video_Video {
  constructor(instance) {
    this.$el = jquery_default()(instance.el);
    this.$overlayPlay = jquery_default()('[data-overlay-play]', this.$el);
    this.$overlayPlay.on('click.video', event => this.activateVideo(event));
    fitvids_default()(this.$el);
  }
  activateVideo(event) {
    const $target = jquery_default()(event.currentTarget);
    const $videoWrapper = $target.parents('[data-video-wrapper]');
    const $overlay = $videoWrapper.find('[data-video-overlay]');
    const $video = $videoWrapper.find('iframe');
    if (!$video.length) {
      return;
    }
    $videoWrapper.addClass('active');
    setTimeout(() => $overlay.addClass('inactive'), 200);
    setTimeout(() => {
...
;// CONCATENATED MODULE: ./source/scripts/utils/FlickityTouchFix.js
...
;// CONCATENATED MODULE: ./source/scripts/Launch.js
...
const initLaunch = () => {
  ...
  const isPassword = document.body.classList.contains('template-password');
  const rteViews = [];
  if (!isPassword) {
    $('.rte').map(rte => rteViews.push(new RTE({
      el: rte
    })));
  }
  themeUtils.externalLinks($(document));
  ...
  sections.register('pxs-announcement-bar', section => new AnnouncementBar(section));
  ...
  sections.register('dynamic-alternating-content', section => new Video_Video(section), {
    lazy: true
  });
  ...
  sections.register('dynamic-images-with-text', section => new DynamicImagesWithText(section), {
    lazy: true
  });
  sections.register('dynamic-testimonials', section => new DynamicTestimonial(section), {
    lazy: true
  });
  sections.register('faq', section => new Accordion(section), {
    lazy: true
  });
  sections.register('age-gate', section => new PageAgeGate(section));
  if ($('select').length) {
    $('select').each((index, item) => new Select({
      el: $(item)
    }));
  }
...
if (checkPolyfills.length) {
  script_default()(checkPolyfills, initLaunch);
} else {
  initLaunch();
}
...
/******/ })()
;
//# sourceMappingURL=launch.js.map?1673904998548